...
class Palette:
    ...
    player_dash: pygame.Color = field(default_factory=lambda: pygame.Color("#b48ead"))
    ...
    powerup: pygame.Color = field(default_factory=lambda: pygame.Color("#a3be8c"))
...
class Player(pygame.sprite.Sprite):
    ...
    def is_invincible(self) -> bool:
        return self.invincible_for > 0 or self.is_dashing
...
class Game:
    ...
    def _reset_level(self, *, keep_state: bool = False) -> None:
        ...
        for __ in range(100):
            x = rng.randint(self.playfield.left + 40, self.playfield.right - 40)
            y = rng.randint(self.playfield.top + 40, self.playfield.bottom - 40)
            powerup = Powerup((x, y), color=self.palette.powerup)

            if pygame.sprite.spritecollideany(powerup, self.walls):
                continue
            if pygame.sprite.spritecollideany(powerup, self.coins):
                continue
            if pygame.sprite.spritecollideany(powerup, self.powerups):
                continue
    ...
    def handle_event(self, event: pygame.event.Event) -> None:
        ...
        if event.key == pygame.K_LSHIFT and self.state == "play" and self.player.has_dash and not self.player.is_dashing:
            self._start_dash()
    ...
    def _start_dash(self) -> None:
        ...
        if move.length_squared() > 0:
            self.player.dash_direction = move
        else:
            # Default to facing right if no input
            self.player.dash_direction = pygame.Vector2(1, 0)
    ...
    def _apply_damage(self, source_rect: pygame.Rect) -> None:
        ...
        # If player has dash powerup, lose it instead of taking damage
        if self.player.has_dash:
            self.player.has_dash = False
    ...
    def update(self, dt: float) -> None:
        ...
        if self.player.is_dashing:
            ...
            if self.player.dash_timer <= 0:
                ...
                dash_vel = self.player.dash_direction * self.player.dash_speed
                self._move_player_axis("x", dash_vel.x * dt)
                self._move_player_axis("y", dash_vel.y * dt)
        else:
            # Normal movement
        ...
        # Triggers: powerup pickup
        powerup_picked = pygame.sprite.spritecollide(self.player, self.powerups, dokill=True)
        if powerup_picked:
    ...
    def draw(self) -> None:
        ...
        # Draw powerups (bigger art than hitbox)
        for powerup in self.powerups:
            visual = pygame.Rect(0, 0, powerup.visual_size, powerup.visual_size)
            visual.center = powerup.rect.center
            # Draw star-like shape for powerup
            center = visual.center + cam
            pts = []
            for i in range(10):
                ...
                pts.append((x, y))
            pygame.draw.polygon(self.screen, powerup.color, pts)
        ...
        # Change color when dashing or has dash powerup
        if self.player.has_dash:
            player_color = self.palette.player_dash

        if self.player.is_invincible:
            # Simple blink while invincible
            if self.player.is_dashing:
                # During dash, don't blink (always show)
                pass
            elif int(self.player.invincible_for * 16) % 2 == 0:
    ...
    def _draw_debug(self, cam: pygame.Vector2) -> None:
        ...
        for powerup in self.powerups:
            pygame.draw.rect(self.screen, pygame.Color("#a3be8c"), powerup.rect.move(cam), 2)
        for hazard in self.hazards:
