...
class Game:
    ...
    def _reset_run(self) -> None:
        ...
        self.score = 0
        self.alive_time = 0.0
        self.time_until_next_spawn = 5.0  # First enemy spawns after 5 seconds

        self.enemy_rects: list[pygame.Rect] = []
        self.enemy_vs: list[pygame.Vector2] = []
        # Start with 2 smaller enemies
        for _ in range(2):
            self._spawn_enemy()
    ...
    def _spawn_enemy(self) -> None:
        """Spawn a new enemy at a random position with random velocity."""
        r = pygame.Rect(
            random.randrange(40, self.w - 40),
            random.randrange(80, self.h - 40),
            24,  # Smaller width (was 36)
            24   # Smaller height (was 36)
        )
        # Moderate, consistent speed
        v = pygame.Vector2(
            random.choice([-1, 1]) * random.randrange(180, 220),
            random.choice([-1, 1]) * random.randrange(150, 190)
        )
    ...
    def update(self, dt: float) -> None:
        ...
        if self.time_until_next_spawn <= 0:
            self._spawn_enemy()
            # Spawn interval decreases over time but caps at 3 seconds
            # Starts at 5s, gradually decreases to 3s minimum
            spawn_interval = max(3.0, 5.0 - (self.alive_time / 30.0))
    ...
    def _draw_hud(self) -> None:
        ...
        text = f"Score: {self.score}    High: {self.high_score}    Enemies: {len(self.enemy_rects)}"
